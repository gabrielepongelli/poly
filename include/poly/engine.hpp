...
namespace poly {
    ...
    class PolymorphicEngineBase : public impl::Crtp<Real> {
      public:
        ...
        //!
        template <std::uint8_t word_size = kByteWordSize>
        void encrypt_code(const EncryptionSecret<word_size> &secret) noexcept;
    ...
    //! - the Compiler class template parameter is used for the generation of
    //! the assembly code and its serialization. It must inherit from
    //! poly::Compiler
    ...
    //!
    //! This is a generic implementation for not supported operating systems. In
    //! order to support an os, a specialization for the appropriate HostOS
    ...
    class PolymorphicEngine
        : public PolymorphicEngineBase<
              PolymorphicEngine<Cipher, Compiler, Editor, OS>, Cipher, Compiler,
              Editor> {
        static_assert(OS != HostOS::kNotSupported, "");
    };

    //!
    //! \typedef OsPolymorphicEngine
    //!
    //! PolymorphicEngine implementation. This class will automatically be
    //! resolved to the specific PolymorphicEngine implementation to use
    ...
    using OsPolymorphicEngine =
        PolymorphicEngine<Cipher, Compiler, Editor, kOS>;
